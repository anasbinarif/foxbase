...
async function main() {
    ...
    try {
        ...
        await client.query('INSERT INTO questionnaires(id, name) VALUES ($1, $2) ON CONFLICT (id) DO NOTHING', [QUESTIONNAIRE_ID, 'Example Insulation Finder']);
        for (const q of QUESTIONS) {
            await client.query('INSERT INTO questions(id, questionnaire_id, position, text) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING', [q.id, q.questionnaireId, q.position, q.text]);
        }
        for (const o of ANSWER_OPTIONS) {
            await client.query('INSERT INTO answer_options(id, question_id, label) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING', [o.id, o.questionId, o.label]);
        }
        for (const e of ANSWER_EFFECTS) {
            await client.query('INSERT INTO answer_effects(id, answer_option_id, type, payload) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING', [e.id, e.answerOptionId, e.type, e.payload]);
        }
        for (const p of PRODUCTS) {
            await client.query('INSERT INTO products(id, name, category, price_cents, attributes) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (id) DO NOTHING', [p.id, p.name, p.category, p.priceCents, p.attributes]);
        }
        await client.query('COMMIT');
        logger_1.logger.info(' demonstration data seeded');
    }
    catch (err) {
